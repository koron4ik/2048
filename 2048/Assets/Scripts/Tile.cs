...
public class GameManager : MonoBehaviour 
{
    ...
    void MoveTile(Tile tile, int index)
    {
        tiles[index] = Instantiate(tile, gameField.emptyTiles[index].transform.position, Quaternion.identity, gameField.emptyTiles[index].transform);
    }
    ...
    IEnumerator MoveTilesDown()
    {
        for (int j = 0; j < 4; j++)
        {
            for (int i = 2; i >= 0; i--)
            {
                ...
                if (tiles[curIndex] != null && tiles[newIndex] == null)
                {
                    MoveTile(tiles[curIndex], newIndex);
                    Destroy(tiles[curIndex].gameObject);
                    tiles[curIndex] = null;
                }
                else if (tiles[newIndex] != null && tiles[curIndex] != null)
                {
                    if (CanMerge(curIndex, newIndex))
                    {
                        Destroy(tiles[newIndex].gameObject);
                        tiles[newIndex] = null;

                        AddNewTile(tiles[curIndex].number * 2, newIndex);
